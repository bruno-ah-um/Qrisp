...
from xdsl.dialects.func import FlatSymbolRefAttrConstr
...
class JaspCallOp(IRDLOperation):
    """Calls a quantum kernel (func.func with QuantumState in its signature).

    From the caller's perspective the operation is purely classical — no
    QuantumState appears in operands or results.  The callee is a regular
    ``func.func`` whose last input/output is ``!jasp.QuantumState``; the
    QuantumState lifecycle is an implementation detail managed at the call
    boundary.
    """
